...
class AddThemaForm(forms.ModelForm):
    ...
    img = forms.ImageField(widget=forms.FileInput(attrs={'placeholder':''}), label='∙ 업로드할 사진 선택')
    private = forms.BooleanField(widget=forms.CheckboxInput(attrs={'placeholder':''}), label='∙ 비공개 테마로 작성하시겠습니까?')
    class Meta:
        model = Thema
